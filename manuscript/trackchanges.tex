...
%DIF DEL sex-bias_revision.tex         Fri Aug 30 12:25:14 2019
%DIF ADD sex-bias_revision_final.tex   Fri Sep 27 14:51:15 2019
% Preamble
%-------------------------------------------------------------------------------
...
\author{
  ...
  Kristofer M. Helgen\DIFdelbegin \DIFdel{$^{4}$}\DIFdelend \DIFaddbegin \DIFadd{$^{1,3,4}$}\DIFaddend , 
}
\date{}
...
Although biological sex may influence all of these areas, it is often overlooked in large-scale studies using \DIFaddbegin \DIFadd{museum }\DIFaddend specimens. 
...
No large-scale study of sex ratios in \DIFaddbegin \DIFadd{bird and mammal }\DIFaddend museum collections exists, therefore investigating this is of vital importance as the number of studies using museum specimens \DIFdelbegin \DIFdel{continue }\DIFdelend \DIFaddbegin \DIFadd{continues }\DIFaddend to rise (e.g. this recent special issue on using museum specimens to study biodiversity in the Anthropocene\cite{meineke2018biological}).
...
In particular a major suspected source of male bias in collections for some species is deliberate selection for large, ``impressive'' male specimens, especially where males are larger or more colourful than females, or possess ornaments \DIFaddbegin \DIFadd{or weaponry }\DIFaddend such as horns or antlers.
...
Alternatively, male bias may be accidental, for example due to trapping biases (i.e. trapping method, season of collecting, conspicuous male behaviors or traits)\DIFaddbegin \DIFadd{, difficulties identifying females to species-level, }\DIFaddend or in some cases simply because there were more males in a population. 
...
To investigate these biases further, we tested whether male bias differed among orders, with sexual size dimorphism, with the possession of colourful plumage (birds) or ornamentation \DIFaddbegin \DIFadd{or weaponry }\DIFaddend (mammals) in males, and through time. 
...
%DIFDELCMD <   \input{figures/table-consequences_revision}
...
(v) Type status. We split types into name bearing (Holotype, Syntype, Lectotype, Neotype) and non-name bearing (\DIFdelbegin \DIFdel{Paratype, Paralectotype, Paraneotype, Allotype, Topotype, Cotype}\DIFdelend \DIFaddbegin \DIFadd{all others}\DIFaddend ) types. Where the records did not specify the kind of type we define these as ambiguous types; 
...
For mammals, we used the Handbook of Mammals of the World to identify mammals where males have \DIFdelbegin \DIFdel{ornamentation}\DIFdelend \DIFaddbegin \DIFadd{``ornamentation''}\DIFaddend . 
...
Reproducible scripts are available on GitHub at https://github.com/nhcooper123/sex-bias-museums\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{we will add a zenodo link on acceptance}%DIFDELCMD < ]%%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{coopercode2019}}%DIFAUXCMD
}\DIFaddend .
...
For mammals, the number of non-sexed individuals was much lower at 15\%, likely because it is \DIFaddbegin \DIFadd{often }\DIFaddend easier to identify sex in mammals, with 41\% female and 44\% male specimens. 
...
%DIFDELCMD <  \centering
%DIFDELCMD <   \includegraphics[width = \linewidth]{figures/orders-density-birds-six.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Kernel density plots showing the \% female specimens in each species across the six largest orders of birds (from left to right, top to bottom: Passeriformes, Apodiformes, Piciformes, Psittaciformes, Charadriiformes, and Columbiformes). 
  ...
  %DIFAUXCMD
%DIFDELCMD < \label{fig-bird_order_six}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Variation among orders.} 
...
In the the past, bats were often trapped by collecting all individuals in a roost site, and female bats \DIFaddbegin \DIFadd{may }\DIFaddend use fewer roost sites than males (e.g. in \textit{Myotis daubentonii}\cite{encarnaccao2012spatiotemporal}), so skew towards females is not surprising. 
The most male-biased order of mammals were the even-toed ungulates (Artiodactyla; 39.7\% females), but although they exhibit a great deal of variation in adult sex ratio, on average, there are more females than males in wild populations\cite{berger1999sex} suggesting strong selection for male specimens in this order derived from the \DIFdelbegin \DIFdel{trophy }\DIFdelend \DIFaddbegin \DIFadd{deliberate }\DIFaddend hunting of large males that was common in the 19th and early 20th centuries.

%DIF <  figure 2
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <  \centering
%DIFDELCMD <   \includegraphics[width = \linewidth]{figures/orders-density-mammals-six.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Kernel density plots showing the \% female specimens in each species across the six largest orders of mammals (from left to right, top to bottom: Rodentia, Chiroptera, Soricomorpha, Carnivora, Primates, and Artiodactyla). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
  ...
  %DIFAUXCMD
%DIFDELCMD < \label{fig-mammal_order_six}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Changes through time.}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <  \centering
%DIFDELCMD <   \includegraphics[width = \linewidth]{figures/plumage.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Relationship between the percentage of female specimens for each species and log plumage dimorphism scores in passerine birds. 
  Only species with at least 100 specimens are included. 
  ...
  %DIFAUXCMD
%DIFDELCMD < \label{fig-plumage}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend For mammals, species with ornaments (horns, antlers, tusks, manes etc.) had significantly fewer female specimens than those without ornaments (Figure \ref{fig-orn}; $F_{1, 1510} = 46.98$, $p < 0.001$; $slope \pm SE = -0.328 \pm 0.048$). 
Note, however, that only 67 species across four orders in our analyses had ornamentation (Table A6).
Most of the species with ornaments in our models were artiodactyls, and \DIFdelbegin \DIFdel{all }\DIFdelend \DIFaddbegin \DIFadd{most }\DIFaddend artiodactyls have horns, antlers or tusks, suggesting that the strong male bias in Artiodactyla (39.7\% female; Figure \ref{fig-mammal_order_six}) was due to selection for males with these features. 
...
We note, however, that artiodactyls only make up 4\% of our dataset, and just over 4\% of all mammal species\cite{wilson2005mammal}, so while \DIFdelbegin \DIFdel{trophy hunting }\DIFdelend \DIFaddbegin \DIFadd{deliberate hunting of large males }\DIFaddend may be a driver in this order, it is unlikely to drive all male biases we see.

%DIF <  figure 4
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <  \centering
%DIFDELCMD <   \includegraphics[width = \linewidth]{figures/ornamentation.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Kernel density plots comparing the \% female specimens in each mammal species where ornaments, i.e. horns, tusks, antlers, manes etc., are present or absent (top panel), and when species have ornaments, whether these are found in both sexes or only males (bottom panel). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig-orn}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Type specimens.} 
...
\DIFaddbegin \section{\DIFadd{Data accessibility}}\label{data-code-and-materials}
\DIFadd{Data are available from the NHM Data Portal \mbox{%DIFAUXCMD
\cite{sex-bias-data} }%DIFAUXCMD
and GBIF\mbox{%DIFAUXCMD
\cite{gbif}}%DIFAUXCMD
. 
...
\section{\DIFdelbegin \DIFdel{Data code }\DIFdelend \DIFaddbegin \DIFadd{Tables }\DIFaddend and \DIFdelbegin \DIFdel{materials}\DIFdelend \DIFaddbegin \DIFadd{Figures}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \label{data-code-and-materials}
%DIFDELCMD < %%%
\DIFdel{Data are available from the NHM Data Portal \mbox{%DIFAUXCMD
\cite{sex-bias-data} }%DIFAUXCMD
and GBIF. 
  R code is available from GitHub (https: //github. 
  com/nhcooper123/sex-bias-museums)(Zenodo DOI will be added on acceptance)}\DIFdelend \DIFaddbegin \newpage
\begin{landscape}
...
\begin{figure}[h]
 \centering
  ...
  \caption{\DIFaddFL{Kernel density plots showing the \% female specimens in each species across the six largest orders of birds (from left to right, top to bottom: Passeriformes, Apodiformes, Piciformes, Psittaciformes, Charadriiformes, and Columbiformes). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
  Silhouettes are from PhyloPic.org contributed by Ferran Sayol (parrot, hummingbird, tit), Steven Traver (woodpecker) and Alexandre Vong (shorebird).}}
...
\begin{figure}[h]
 \centering
  ...
  \caption{\DIFaddFL{Kernel density plots showing the \% female specimens in each species across the six largest orders of mammals (from left to right, top to bottom: Rodentia, Chiroptera, Soricomorpha, Carnivora, Primates, and Artiodactyla). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
  Silhouettes are from PhyloPic.org contributed by Daniel Jaron (mouse), Yan Wong (bat), Becky Barnes (shrew), Lukasiniho (tiger), Sarah Werning (monkey), and Oscar Sanisidro (deer).
}}
...
\begin{figure}[h]
 \centering
  ...
  \caption{\DIFaddFL{Relationship between the percentage of female specimens for each species and log plumage dimorphism scores in passerine birds. 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens; the dotted line is the point at which males and females have the same plumage colouration. 
  Plumage dimorphism scores were calculated by dividing male plumage scores by female plumage scores (see Methods). 
}}
...
\begin{figure}[h]
 \centering
  ...
  \caption{\DIFaddFL{Kernel density plots comparing the \% female specimens in each mammal species where ornaments, i.e. horns, tusks, antlers, manes etc., are present or absent (top panel), and when species have ornaments, whether these are found in both sexes or only males (bottom panel). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
}}
  \label{fig-orn}
...
\newpage
\section{\DIFadd{Table and figure legends}}

\DIFadd{Table 1: Prominent uses of natural history specimens and how research outcomes may be influenced by sex biases.
}

\DIFadd{Figure 1: Kernel density plots showing the \% female specimens in each species across the six largest orders of birds (from left to right, top to bottom: Passeriformes, Apodiformes, Piciformes, Psittaciformes, Charadriiformes, and Columbiformes). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
  Silhouettes are from PhyloPic.org contributed by Ferran Sayol (parrot, hummingbird, tit), Steven Traver (woodpecker) and Alexandre Vong (shorebird).
}

\DIFadd{Figure 2: Kernel density plots showing the \% female specimens in each species across the six largest orders of mammals (from left to right, top to bottom: Rodentia, Chiroptera, Soricomorpha, Carnivora, Primates, and Artiodactyla). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens. 
  Silhouettes are from PhyloPic.org contributed by Daniel Jaron (mouse), Yan Wong (bat), Becky Barnes (shrew), Lukasiniho (tiger), Sarah Werning (monkey), and Oscar Sanisidro (deer).
}

\DIFadd{Figure 3: Relationship between the percentage of female specimens for each species and log plumage dimorphism scores in passerine birds. 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens; the dotted line is the point at which males and females have the same plumage colouration. 
  Plumage dimorphism scores were calculated by dividing male plumage scores by female plumage scores (see Methods).
}

\DIFadd{Figure 4: Kernel density plots comparing the \% female specimens in each mammal species where ornaments, i.e. horns, tusks, antlers, manes etc., are present or absent (top panel), and when species have ornaments, whether these are found in both sexes or only males (bottom panel). 
  Only species with at least 100 specimens are included. 
  The dashed line represents 50\% female specimens}\DIFaddend .
