...
```{r}
# Just for the top 6 orders

summarise(group_by(ds_ssd_birds, order), length((which(SSD>0))), n())


ds_ssd_birds2 <- 
  ds_ssd_birds %>%
  filter(order == "Psittaciformes" | order == "Passeriformes" |
  order == "Charadriiformes" | order == "Columbiformes" |
  order == "Apodiformes" | order == "Piciformes")  %>%
  mutate(order = relevel(as.factor(order), ref = "Passeriformes"))
                 
ssd_model_birds2 <- glm(cbind(female, male) ~ log(SSD)*order, 
                        data = ds_ssd_birds2, family = "binomial")

# Look at outputs
tidy(anova(ssd_model_birds2, test = "Chisq"))
summary(ssd_model_birds2)

# Look at model diagnostics
autoplot(ssd_model_birds2)
```
