---
title: "Revision - Genus-level summary stats"
output:
...
specimens <- read_csv(here("data/genera-specimen-data.csv")) 
```
...
## And what if we only look at genera with > 100 specimens

```{r}
  sum3 <-
  specimens %>%
  filter(!is.na(sex)) %>%
  add_count(genus, name = "spec") %>%
  filter(spec >= 100) %>%
  add_count(class, name = "n") %>%
  add_count(class, sex, name = "nn") %>%
  select(class, sex, n, nn) %>%
...
## What is the median proportion of females per genus?

```{r}
  proportion <-
  specimens %>%
  filter(!is.na(sex)) %>%
  add_count(class, genus, name = "n") %>%
  add_count(class, genus, sex, name = "nn") %>%
  select(class, genus, sex, n, nn) %>%
  distinct() %>%
...
## What is the median proportion of females per genus for those with >= 100 specimens only?
